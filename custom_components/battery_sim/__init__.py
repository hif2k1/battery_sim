...
class SimulatedBatteryHandle:
    ...
    def charge_percentage(self) -> float:
        """Return the state of charge as a percentage of the current capacity.

        Derived on read so it always matches the charge state, including
        directly after a restore or a charge state set through the service.
        """
    ...
    def update_battery(self, import_amount, export_amount, solar_amount=0.0):
        ...
        if self.charge_percentage < 2:
            self._sensors[ATTR_STATUS] = MODE_EMPTY
        elif self.charge_percentage > 98:
